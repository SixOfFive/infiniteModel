...
<div class="card">
  ...
  <div class="frm">
    ...
    <div class="fld tog"><input type="checkbox" id="juggler"><label for="juggler" title="Checks every ~60s (and right after an idle-unload frees VRAM) whether the hottest resident model running split across GPU+RAM would now fit entirely in VRAM, and if so promotes it. It engages a barrier (new requests wait), drains that model's in-flight generation, re-places it VRAM-only, then resumes — the client connection just pauses briefly, no reconnect. Embeddings and models that can't fit GPU are skipped. Lets models auto-load hybrid under memory pressure and migrate to full-GPU speed as the busy one out-competes quieter models for VRAM.">Juggler (promote hybrid→VRAM on free)</label></div>
  </div>
