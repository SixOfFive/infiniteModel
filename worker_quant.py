...
def _quant2_linear_cls():
    ...
    if _QUANT_LINEAR2 is None:
        ...
        class QuantLinear2(nn.Module):
            ...
            def forward(self, x):
                fz = getattr(self, "_fused", None)
                # #large-m-naive: mirrors QuantLinear4 — above the prepare-time-benched row
                # threshold the decode-tuned _k2 loses to dequant-once+BLAS; fall through to the
                # naive tail (the self-check's reference numerics). 0/absent = always fused.
                _nm = getattr(self, "_naive_m_min", 0)
                if fz is not None and _nm and self.qweight is not None and _rows(x) >= _nm:
                    fz = None
                if fz is not None:
