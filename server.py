...
# moving the definitions would decouple the idle gate (the ENCODING hazard, state.py). All writers
# mutate them IN PLACE (m4c155 + Inc 4: no rebinds anywhere), which is why the download ROUTES and
# _pull_repo_interruptible could move to downloads.py (code-split Inc 5) — a moved global rebind would decouple from server's name (the ENCODING hazard).
